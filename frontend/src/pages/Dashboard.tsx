...
export function Dashboard() {
  ...
  return <div className="page dashboard">
    ...
    <section className="health-strip">
      ...
      <div><span>CODEX</span><Status ok={data?.codex.authenticated ?? false} label={data?.codex.authenticated ? "已登入" : data?.codex.installed ? "待登入" : "未安裝"}/></div>
    ...
    <div className="metric-grid">
      ...
      <article className="metric"><HardDrive/><span>GPU 記憶體</span><strong>{gpu ? `${(gpu.memory_used_mb / 1024).toFixed(1)} GB` : "N/A"}</strong><small>{gpu ? `共 ${(gpu.memory_total_mb / 1024).toFixed(1)} GB` : "STT 將使用 CPU"}</small></article>
      ...
      <article className="metric"><Bot/><span>Codex Queue</span><strong>0</strong><small>無等待中的推理工作</small></article>
    </div>
    <div className="dashboard-grid">
      ...
      <section className="section system-list"><div className="section-title"><div><p className="eyebrow">SYSTEM</p><h2>執行環境</h2></div></div>
        {[ ["作業系統", data?.os], ["Python", data?.python_version], ["Codex CLI", data?.codex.version ?? "未偵測"], ["FFmpeg", data?.ffmpeg_available ? "可用" : "未安裝"], ["磁碟空間", data ? `${data.disk.free_gb} GB 可用` : "—"] ].map(([k,v]) => <div className="system-row" key={k}><span>{k}</span><strong>{v ?? "檢查中"}</strong></div>)}
      </section>
    </div>
  </div>;
}
