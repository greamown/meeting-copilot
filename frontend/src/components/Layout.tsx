...
export function Layout() {
  ...
  const actions=useQuery({queryKey:["actions"],queryFn:()=>getActions(),staleTime:60_000,refetchInterval:300_000});
  const now=Date.now();
  const overdue=(actions.data??[]).filter(row=>row.due_at&&new Date(row.due_at).getTime()<now&&!closed.includes(row.status)).length;
  return <div className="shell">
    <aside className="sidebar">
      ...
      <nav aria-label="主要導覽">{nav.map(([to, label, Icon]) => <NavLink key={to} to={to} end={to === "/"} title={t(label)} aria-label={t(label)}><Icon size={18}/><span>{t(label)}</span>{to === "/actions" && overdue > 0 && <span className="nav-badge" title={`${overdue} 項行動項目已逾期`}>{overdue}</span>}</NavLink>)}</nav>
