...
class Cursor:
    ...
    async def execute(self, operation: str, parameters: Any = None) -> "Cursor":
        self._cursor.execute(operation, () if parameters is None else parameters)
        return self

    async def executemany(self, operation: str, parameters: Any) -> "Cursor":
        self._cursor.executemany(operation, parameters)
        return self
    ...
    async def fetchmany(self, size: int | None = None):
        return self._cursor.fetchmany(size or 1)
