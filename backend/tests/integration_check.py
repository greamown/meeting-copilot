...
integration_db = Path("/tmp/meeting-copilot-integration.db")
integration_db.unlink(missing_ok=True)
os.environ["MC_DATABASE_URL"] = f"sqlite+aiosqlite:///{integration_db}"
    ...
    async with app.router.lifespan_context(app):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            ...
            created = await client.post(
                "/api/meetings",
                json={"title": "Integration", "goal": "Verify flow", "privacy_acknowledged": True},
            )
            ...
            for command, expected in (
                ...
                response = await client.post(f"/api/meetings/{meeting_id}/{command}")
                assert response.status_code == 200, response.text
                assert response.json()["meeting"]["status"] == expected
            run = await client.post(
                f"/api/meetings/{meeting_id}/ask", json={"question": "What is missing?"}
            )
            ...
            completed = await wait_status(run.json()["run_id"], {"completed", "failed"})
            assert completed["codex_runs"][0]["status"] == "completed"
            assert completed["suggestions"]
            suggestion_id = completed["suggestions"][0]["id"]
            accepted = await client.post(f"/api/suggestions/{suggestion_id}/accept")
            assert accepted.json()["status"] == "accepted"
            os.environ["MOCK_CODEX_MODE"] = "invalid"
            invalid = await client.post(f"/api/meetings/{meeting_id}/analyze")
            ...
            response = await client.post(f"/api/meetings/{meeting_id}/end")
            assert response.json()["meeting"]["status"] == "ended"
            for export in ("markdown", "json", "vtt", "srt", "pdf"):
                response = await client.post(f"/api/meetings/{meeting_id}/export/{export}")
                assert response.status_code == 200
                if export == "pdf":
                    assert response.content.startswith(b"%PDF")
            summary = await client.get(f"/api/meetings/{meeting_id}/summary")
            assert summary.status_code == 200
            assert summary.json()["meeting_id"] == meeting_id
            detail = await client.get(f"/api/meetings/{meeting_id}")
            assert detail.status_code == 200, detail.text
            migration = await client.post("/api/diagnostics/migrations")
            assert migration.json()["valid"] is True
    print(
        "integration check passed: lifecycle, Codex success/invalid/timeout/cancel, suggestions, exports, migrations"
    )
